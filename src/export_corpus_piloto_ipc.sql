...
WITH file_text AS (
  SELECT pg_read_file('/tmp/numeros_corpus_piloto.txt') AS content
),
linhas AS (
...
) TO '/tmp/corpus_piloto_bruto.tsv'
...
) TO '/tmp/corpus_piloto_bruto_nao_encontradas.tsv'
WITH (FORMAT text, DELIMITER E'\t', HEADER true, ENCODING 'UTF8');
...
) TO '/tmp/corpus_piloto_bruto_resumo.tsv'
WITH (FORMAT text, DELIMITER E'\t', HEADER true, ENCODING 'UTF8');
