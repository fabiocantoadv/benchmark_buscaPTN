...
A lista de números já está pronta: **`dados/numeros_corpus_piloto.txt`** (1.000
...
docker cp dados/numeros_corpus_piloto.txt patentes-postgres:/tmp/numeros_corpus_piloto.txt

# 2. export com IPC (mesmo SQL da amostra original, só com os caminhos trocados)
docker cp src/export_corpus_piloto_ipc.sql patentes-postgres:/tmp/
...
python3 src/enriquecer_ipc_json_pt.py --entrada corpus_piloto_bruto.tsv \
                                  --saida corpus_piloto_ipc.tsv

# 5. conferir cobertura e regravar o qrels
python3 src/consolidar_corpus_ipc.py corpus_piloto_ipc.tsv
```
...
`dados/corpus_piloto_ipc.tsv` e as colunas `texto_para_embedding`,
`texto_para_embedding_ipc_pt`, `texto_para_embedding_ipc_hierarquia_pt`.

Notas:

- `src/enriquecer_ipc_json_pt.py` agora aceita `--entrada/--saida/--faltantes/--resumo/--ipc-json`;
